...
impl NodeGraph {
    ...
    pub fn merge(&mut self, other: &mut Self) -> HashMap<NodeId, NodeId> {
        ...
        for node_id in self.iter_nodes().collect::<HashSet<_>>().into_iter() {
            if let Some(mut other_node) = other.nodes.remove(node_id) {
                ...
                let mut new_inputs: Vec<(String, InputId)> = self[new_node_id].inputs.clone();
                for (_label, input_id) in new_inputs.iter_mut() {
                ...
                let mut new_outputs: Vec<(String, OutputId)> = self[new_node_id].outputs.clone();
                for (_label, output_id) in new_outputs.iter_mut() {
                ...
                self[new_node_id].inputs = new_inputs;
                self[new_node_id].outputs = new_outputs;
    ...
    pub fn add_input(&mut self, label: String, node_id: NodeId, data: InputData) -> InputId {
        let input_id = self
            .inputs
    ...
    pub fn remove_input(&mut self, input_id: InputId) {
        ...
        self[node_id].inputs.retain(|(_, id)| *id != input_id);
    ...
    pub fn remove_output(&mut self, output_id: OutputId) {
        ...
        self[node_id].outputs.retain(|(_, id)| *id != output_id);
        self.outputs.remove(output_id);
        self.edges.remove_output(output_id);
    }

    pub fn add_output(&mut self, label: String, node_id: NodeId, data: OutputData) -> OutputId {
        let output_id = self
            .outputs
    ...
    pub fn remove_node(&mut self, node_id: NodeId) -> (Node, Vec<(InputId, OutputId)>) {
        ...
        for input in self[node_id].input_ids().collect::<Vec<_>>() {
            self.inputs.remove(input);
        }
        for output in self[node_id].output_ids().collect::<Vec<_>>() {
    ...
    pub fn descendant_output_ids(&self, node_id: NodeId) -> HashSet<OutputId> {
        ...
        while let Some(search_node_id) = nodes_to_search.pop() {
            output_ids.extend(self[search_node_id].output_ids().map(|output_id| {
    ...
    pub fn descendant_node_ids(&self, node_id: NodeId) -> HashSet<NodeId> {
        ...
        while let Some(search_node_id) = nodes_to_search.pop() {
            self[search_node_id].output_ids().for_each(|output_id| {
                if let Some(input_ids) = self.edges.children(output_id) {
                    nodes_to_search
                        .extend(input_ids.iter().map(|input_id| self[*input_id].node_id));
                    descendant_ids.extend(input_ids.iter().map(|input_id| self[*input_id].node_id));
                }
            });
        }
        descendant_ids
    ...
    pub fn ancestor_node_ids(&self, node_id: NodeId) -> HashSet<NodeId> {
        ...
        while let Some(search_node_id) = nodes_to_search.pop() {
            self[search_node_id].input_ids().for_each(|input_id| {
    ...
    pub fn try_get_parent(&self, input: InputId) -> Option<&OutputId> {
        self.edges.parent(input)
    }

    pub fn try_get_children(&self, output: OutputId) -> Option<&HashSet<InputId>> {
        self.edges.children(output)
    }

    pub fn try_get_input(&self, input: InputId) -> Option<&Input> {
        self.inputs.get(input)
    }

    pub fn get_input(&self, input: InputId) -> &Input {
        &self.inputs[input]
    }

    pub fn try_get_output(&self, output: OutputId) -> Option<&Output> {
        self.outputs.get(output)
    }

    pub fn get_output(&self, output: OutputId) -> &Output {
        &self.outputs[output]
    }
}
