...
pub fn show_toolbar(
    ...
    egui::TopBottomPanel::top("toolbar").show(egui_context, |ui| {
        egui::menu::bar(ui, |ui| {
            let mut load_requested: bool = egui_context.input(|i| {
                i.key_pressed(egui::Key::L) && i.modifiers.matches_logically(egui::Modifiers::CTRL)
            });
            let mut save_requested: bool = egui_context.input(|i| {
                i.key_pressed(egui::Key::S) && i.modifiers.matches_exact(egui::Modifiers::CTRL)
            });
            let mut save_as_requested: bool = egui_context.input(|i| {
