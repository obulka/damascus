...
//     viewport::Viewport,
// };
use crate::{EnumTraits, NodeGraph, app::Context};

pub struct FileDescriptor {
    path: String,
}

#[derive(Clone, Default, EnumTraits!)]
...
impl Toolbar {
    pub fn update(&mut self, context: &Context, node_graph: &NodeGraph, message: ToolbarMessage) {
        match message {
            ToolbarMessage::File(file_message) => match file_message {
                FileMessage::Load(_file) => {}
                FileMessage::Save => {
                    save(&context.working_file, node_graph, true);
                }
